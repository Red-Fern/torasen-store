...
class WC_CSP_Admin {
	...
	public function __construct() {
		...
		// Add a notice if no shipping zones are defined.
		add_action( 'admin_notices', array( $this, 'maybe_add_no_shipping_methods_notice' ), 0 );
	...
	*/
	...
	/**
	 * Add a notice if no shipping methods are active.
	 *
	 * @return void
	 */
	public function maybe_add_no_shipping_methods_notice() {
		...
		if ( 0 === $method_count ) {
			$shipping_zones_link = admin_url( 'admin.php?page=wc-settings&tab=shipping' );
			$learn_more_link     = 'https://woocommerce.com/document/setting-up-shipping-zones/';
			WC_CSP_Admin_Notices::add_notice( sprintf(
				__( 'Your store does not have any active <a href="%1$s">Shipping Methods</a>. Before setting up Shipping Methods Restrictions, it is necessary to create or activate some Shipping Methods. <a href="%2$s">Learn more</a> about setting up Shipping Zones and Shipping Methods for your store.', 'woocommerce-conditional-shipping-and-payments' ),
				$shipping_zones_link,
				$learn_more_link
			), 'warning', false );
		}
	...
	*/
	...
	public static function plugin_row_meta( $links, $file ) {
		...
		if ( WC_CSP()->get_plugin_basename() === $file ) {
			$row_meta = array(
				'docs'    => '<a href="https://woocommerce.com/products/conditional-shipping-and-payments/">' . __( 'Documentation', 'woocommerce-conditional-shipping-and-payments' ) . '</a>',
				'support' => '<a href="https://woocommerce.com/my-account/marketplace-ticket-form/">' . __( 'Support', 'woocommerce-conditional-shipping-and-payments' ) . '</a>',
			);
